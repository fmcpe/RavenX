...
public class HUD extends Module {
            ...
            for (Module m : en)
                if (m.isEnabled() && m.showInHud()) {
                    ...
                    if (ColourModes.values()[(int) colourMode.getInput() - 1] == ColourModes.RAVEN) {
                        ...
                        y += mc.fontRendererObj.FONT_HEIGHT + margin + 1;
                        del -= 30;
                    } else if (ColourModes.values()[(int) colourMode.getInput() - 1] == ColourModes.PURPLE) {
                        color = Utils.Client.purpledraw(del);
                        y += mc.fontRendererObj.FONT_HEIGHT + margin + 1;
                        del -= 30;
                    }
                    else if (ColourModes.values()[(int) colourMode.getInput() - 1] == ColourModes.RED) {
                        color = Utils.Client.reddraw(del);
                        y += mc.fontRendererObj.FONT_HEIGHT + margin + 1;
                        del -= 30;
    ...
    public enum ColourModes {
        RAVEN, PURPLE , RED , RAVEN2, ASTOLFO, ASTOLFO2, ASTOLFO3, KV
    }
